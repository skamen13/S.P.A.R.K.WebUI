...
async function StartAI(user, socket, question, systemQuestion = false) {
    ...
    if (S_E_A_R_C_H !== ""){
        ...
        const finalAnswerChat = await groq.chat.completions.create({
            "messages": messagesParam,
            "model": "llama-3.1-70b-versatile",
            "temperature": 1,
            "max_tokens": 1024,
...
function separateSystemMessages(messages) {
    // Массив для системных сообщений, начинающихся на "Ответ из поиска:\n"
    ...
    messages.forEach(message => {
        ...
        if (message.role === 'system' && message.content.startsWith('Ответ из поиска:\n')) {
